/**
 * Created by vaikunth on 2/29/16.
 */
// ==ClosureCompiler==
...
;(function()
{
    ...
    root.Inrix = {
         ...
         * @method userRegister
         *
         * @description: Login or register the user using provided credentials
         *
         * @param options.userId user id for the user login
         *
         * @param options.password password for the user login
        ...
        deviceRegister: function(options) {
            ...
            options.systemVersion = this.configuration.systemVersion || this.VERSION;
            ...
            if (options.action === 'Mobile.User.Register') {
                tokens = tokens.concat(registerTokens);
            }

            tokens.push(this.configuration.vendorToken);

            var tokenizedString = tokens.join("|").toLowerCase();
            options.token = this.tokenizer(tokenizedString);
...
//            }
        ...
        parseErrorResults: function(jqXHR, errorText, error) {
            try {
                var errorMessage = error || jqXHR.responseText || jqXHR.responseXML;
                console.log('[' + errorMessage + '] - ' + JSON.stringify(jqXHR));
            } catch (e) {
    ...
    Inrix.Incident = function (xml) {
        ...
        if ($community.length > 0) {
            this.community = {};
            this.community.accuracy = $community.attr('accuracy');
            var $contributor = $community.find('Contributor');
            if ($contributor.length > 0) {
                this.community.contributor = {};
                this.community.contributor.displayName = $contributor.attr('displayName');
                this.community.contributor.trustLevel = $contributor.attr('trustLevel');
            }
        }
    };
    ...
    Inrix.Route = function(xml) {
        ...
        var points = [];
        var xmlPointsRoot = $xml.find('Points');
        if (xmlPointsRoot.length > 0) {
            var xmlPoints = xmlPointsRoot[0].childNodes;
            var length = xmlPoints.length;
            for (var i = 0; i < length; i++) {
                if (xmlPoints[i] && xmlPoints[i].nodeName && xmlPoints[i].nodeName === 'Point') {
                    var $this = $(xmlPoints[i]);
    ...
    Inrix.Util = {
        ...
        /**
         * Calculates a distance between points in miles.
         *
         * @method distanceBetweenPointsInMiles
         * @param start Start location object.
         * @param end
         * @return {Number}
         */
        distanceBetweenPointsInMiles: function(start, end) {
            var METERS_PER_MILES = 1609.344;
            var R = 6378137;
            var radian = Math.PI / 180;
            var deltaLatitude = (end.latitude - start.latitude) * radian;
            var deltaLongitude = (end.longitude - start.longitude) * radian;

            var sinLatitude = Math.sin(deltaLatitude / 2);
            var haversineLatitude = sinLatitude * sinLatitude;

            var sinLongitude = Math.sin(deltaLongitude / 2);
            var haversineLongitude = sinLongitude * sinLongitude;

            var latitudeAngle1 = start.latitude * radian;
            var latitudeAngle2 = end.latitude * radian;
            var a = haversineLatitude + haversineLongitude * Math.cos(latitudeAngle1) * Math.cos(latitudeAngle2);

            var distance = R * 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
            var distanceMiles = distance / METERS_PER_MILES;
    ...
    Inrix.IncidentManager = function(options) {
         ...
         * @param options.incidentVersion Comes back form the call
         */
        this.reportAccident = function(options) {
            options.eventCode = 201;
            options.incidentType = 4;
            this.reportIncident(options);
        };
         ...
         * @param options.incidentVersion Comes back form the call
         */
        this.reportHazard = function(options) {
            options.eventCode = 901;
            options.incidentType = 8;
            this.reportIncident(options);
        };

        /**
         * @method reportConstruction
         * @param {Object} options
         * @param {String} options.location Pipe separated lat|long pair:  34.03928|-132.94820.
         * @param options.incidentId Comes back form the call
         * @param options.incidentVersion Comes back form the call
         */
        this.reportConstruction = function(options) {
            options.eventCode = 701;
            options.incidentType = 1;
            this.reportIncident(options);
        };
         ...
         * @param options.eventCode AccidentCode = 201,<br/>EventCode = 701,<br/>PoliceSpeedTrapCode = 1477,<br/>FixedCameraTrapCode = 1971
         * @param {String} options.location Pipe separated lat|long pair:  34.03928|-132.94820.
         * @param options.incidentId Comes back form the call
         * @param options.incidentVersion Comes back form the call
         * @return $.Deferred
